...
class _SignatureCustomWidgetState extends State<SignatureCustomWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    if (selectedIndex != null) {
      ...
      String? json = dataRow[dataChunk.columnDefinitions.indexByName(COLUMN_NAME_SIGNATURE_POINTS)];
        ...
        child: Column(
          children: [
            ...
            Container(
                ...
                child: Column(
                  children: [
                    Container(margin: const EdgeInsets.only(top: 20, bottom: 15),
                      decoration: BoxDecoration(
                        border: Border.all(color: Theme.of(context).colorScheme.primary),
                        color: Colors.white,
                      ),
                      child: SizedBox(height: 250, child: Signature(
                          ...
                          child: IconButton(
                            ...
                            onPressed: () async {
                              await DataBook.updateRecord(
                                  ...
                                  pColumnNames: [COLUMN_NAME_SIGNATURE, COLUMN_NAME_SIGNATURE_POINTS],
                                  pValues: [null, null]);
                            ...
                            onPressed: () async {
                              if (_controller.isNotEmpty) {
                                ...
                                await DataBook.updateRecord(
                                  pDataProvider: CONTACT_DATA_PROVIDER,
                                  pColumnNames: [COLUMN_NAME_SIGNATURE, COLUMN_NAME_SIGNATURE_POINTS],
                                  pValues: [base64Encode(data!), _toJson(_controller.points)],
                                );
                              }
                            },
                          ),
  ...
  String? _toJson(List<Point> points) {
    ...
    points.forEach((point) {
      json.add({'point': {'x': point.offset.dx, 'y': point.offset.dy},
  ...
  List<Point>? _fromJson(String json) {
    ...
    listJson.forEach((entry) {
      result.add(Point(Offset(entry["point"]["x"], entry["point"]["y"]),
